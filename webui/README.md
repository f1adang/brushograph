...
line. An em dash in a comment is not a typographical detail to those boards: it
is a character they act on.

That rule is now kept by the one step that turns text into a file rather than
by remembering it at every comment. `to_ascii` translates the punctuation this
repo writes prose with — em and en dashes, ellipses, curly quotes, ×, °, ± —
decomposes the rest the way NFKD does, drops the combining marks that leaves
behind so an accent costs its mark and not its letter (naive, not nai?ve), and
replaces whatever survives with a question mark. A question mark in a comment
...
from a config is a tray name. That is the other half of the reason the pass is
central rather than a fix to nine string literals — a figure or a name out of a
config can carry anything at all.

The 𝔐𝔦𝔨𝔯𝔬 is the pleasant case: NFKD knows its mathematical fraktur, so a
config naming that model writes `Mikro` into the file rather than five question
marks.
